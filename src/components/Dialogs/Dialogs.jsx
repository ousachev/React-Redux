...
const Dialogs = ({
  dialogsPage: { dialogData, messagesData, },
  sendNewMessage
}) => {
  const dialogArrays = dialogData.map(dialog => {
    return <Dialog name={dialog.name} id={dialog.id} key={dialog.id} />;
  });
  const messagesArrays = messagesData.map(message => {
    return <Message text={message.text} id={message.id} key={message.id} />;
  });

  const { dialogs, users, messages } = classes;
